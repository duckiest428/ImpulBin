            ...
            <div className="max-w-7xl mx-auto flex flex-col gap-6 relative z-10">
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        {/* Chart Container */}
                        <div className="bg-[#1e1e1e] border border-[#2d2d2d] rounded-lg p-6 flex flex-col h-[400px] relative">
                            ...
                            {/* Mock Chart Area */}
                            <div className="flex-1 border-l border-b border-[#333] relative">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                        {mockGainers.map((g, i) => (
                                            <div key={i} className="flex flex-col items-center gap-1">
                                                <div className="w-16 h-20 bg-yellow-900 border border-yellow-500 rounded relative mb-2"></div>
                                                <span className="text-green-500 font-bold text-xs">{g.change}</span>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                        {mockLosers.map((l, i) => (
                                            <div key={i} className="flex flex-col items-center gap-1">
                                                <div className="w-16 h-20 bg-gray-800 border border-gray-600 rounded relative mb-2"></div>
                                                <span className="text-red-500 font-bold text-xs">{l.change}</span>
                                                <span className="text-white font-bold text-xs flex items-center gap-1">
                    ...
                    <div className="w-full lg:w-80 flex flex-col gap-6">
                        ...
                        {/* Index List */}
                        <div className="bg-[#111] border border-[#2d2d2d] rounded overflow-hidden">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-black text-xs font-bold text-white border-b border-[#333]">
                                ...
                                <tbody className="text-sm font-semibold">
                                    {[
                                        { name: 'Index 100', last: 184.09, chg: '+1.62%', color: 'text-green-500', isGreen: true },
                                        { name: 'Index 86', last: 2.74, chg: '-3.86%', color: 'text-red-500', isGreen: false },
                                        { name: 'Index 85', last: 17.93, chg: '+2.75%', color: 'text-green-500', isGreen: true },
                                        { name: 'Index 84', last: 33.94, chg: '-15.32%', color: 'text-red-500', isGreen: false },
                                        { name: 'Index 83', last: 83.64, chg: '+0.08%', color: 'text-green-500', isGreen: true },
                                        { name: 'Index 82', last: 280.95, chg: '+4.79%', color: 'text-green-500', isGreen: true },
                                        { name: 'Index 81', last: 236.52, chg: '-3.72%', color: 'text-red-500', isGreen: false },
                                        { name: 'Index Icons', last: 22.41, chg: '-0.22%', color: 'text-red-500', isGreen: false },
                                    ].map((row, i) => (
                                        <tr key={i} className={`border-b border-[#222] ${i === 0 ? 'bg-[#1a1a1a]' : 'hover:bg-[#1a1a1a]'} transition-colors cursor-pointer`}>
                                            ...
                                            <td className="p-3 text-white">{row.last.toFixed(2)}</td>
                                            <td className={`p-3 text-right ${row.color}`}>
                        ...
                        {/* Market Momentum */}
                        <div className="bg-[#1e1e1e] border border-[#2d2d2d] rounded p-4 text-center">
                            <h3 className="text-gray-400 text-xs font-bold uppercase mb-4">Market Momentum</h3>
                            <div className="bg-green-600 text-white font-bold text-xl py-3 rounded mb-2">53.31%</div>
                            <p className="text-sm text-green-500">The market is starting to wake up</p>
                        </div>
